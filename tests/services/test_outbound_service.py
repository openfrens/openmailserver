...
def test_send_outbound_message_marks_failures(db_session, monkeypatch):
    ...
    monkeypatch.setattr(outbound_service, "get_settings", lambda: settings)
    monkeypatch.setattr(outbound_service.smtplib, "SMTP_SSL", _BrokenSMTP)
    monkeypatch.setattr(outbound_service, "mailbox_password_for", lambda address: "secret-pass")
    monkeypatch.setattr(
        outbound_service,
        "write_system_log",
    ...
    record = outbound_service.send_outbound_message(
        ...
        subject="Hello",
        text_body="World",
        authenticated_sender="agent@example.test",
    )
    db_session.commit()
