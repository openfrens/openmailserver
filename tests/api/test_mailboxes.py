...
from openmailserver.services.mailbox_reader_service import MailboxReadError
...
def test_get_mailbox_messages_maps_reader_errors(monkeypatch):
    monkeypatch.setattr(
        mailboxes,
        "list_messages",
        lambda address: (_ for _ in ()).throw(MailboxReadError("imap unavailable")),
    )

    with pytest.raises(mailboxes.HTTPException) as exc:
        mailboxes.get_mailbox_messages("agent@example.test", _=object())

    assert exc.value.status_code == 503


def test_create_mailbox_maps_runtime_not_ready_to_503(db_session, monkeypatch):
    monkeypatch.setattr(
        mailboxes,
        "provision_mailbox",
        lambda db, payload: (_ for _ in ()).throw(
            MoxRuntimeNotReadyError("mox quickstart missing")
        ),
    )
...
def test_create_alias_maps_runtime_sync_error_to_400(db_session, monkeypatch):
    monkeypatch.setattr(
        mailboxes,
        "create_alias_record",
        lambda db, payload: (_ for _ in ()).throw(MoxSyncError("unsupported alias destination")),
    )

    with pytest.raises(mailboxes.HTTPException) as exc:
        mailboxes.create_alias(
            AliasCreate(source="alias@example.test", destination="user@gmail.com"),
            db=db_session,
            _=object(),
        )
