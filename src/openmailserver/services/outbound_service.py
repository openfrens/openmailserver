...
def send_outbound_message(
    ...
    try:
        if settings.transport_mode == "maildir":
            ...
            smtp_security = getattr(settings, "smtp_security", "plain").lower()
            smtp_factory = smtplib.SMTP_SSL if smtp_security == "ssl" else smtplib.SMTP
            context = ssl.create_default_context()
            if not getattr(settings, "smtp_verify_tls", True):
            ...
            with smtp_factory(
                ...
                **({"context": context} if smtp_security == "ssl" else {}),
            ) as smtp:
                if smtp_security == "starttls":
                    smtp.starttls(context=context)
                smtp_address = authenticated_sender or sender
                smtp_password = mailbox_password_for(smtp_address)
                if smtp_password:
