...
class Settings(BaseSettings):
    ...
    mox_binary: str = "mox"
    mox_admin_account: str = "admin"
    mox_admin_address: str = "admin@example.com"
    ...
    @property
    def effective_mox_admin_address(self) -> str:
        if self.mox_admin_address and self.mox_admin_address != "admin@example.com":
            return self.mox_admin_address
        return f"{self.mox_admin_account}@{self.primary_domain}"

    @property
    def effective_imap_host(self) -> str:
        return self.imap_host or self.canonical_hostname

    @property
    def smtp_ssl_cert_reqs(self):
        return CERT_REQUIRED if self.smtp_verify_tls else CERT_NONE

    @property
    def imap_ssl_cert_reqs(self):
        return CERT_REQUIRED if self.imap_verify_tls else CERT_NONE
